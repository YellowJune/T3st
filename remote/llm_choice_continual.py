#!/usr/bin/env python3
"""Full-FP32 DFC-Sign multiple-choice adaptation on Qwen2.5-0.5B-Instruct.

Qwen's own next-token likelihood over A/B/C/D is optimized with FP32 LoRA and
ordinary FP32 Adam moments. Dataset-specific option meanings are stated in each
prompt. A fixed token-and-dark-logit record is stored either in an external
byte envelope or in that envelope composed with the exact Adam second-moment
sign fiber. Batch shape, updates, dense tokens, model, trainable parameters,
...
TASK_SPECS = (
    {
        ...
        "class_descriptions": ("world", "sports", "business", "science and technology"),
    },
    {
        ...
        "class_descriptions": ("sadness", "joy", "anger", "fear"),
    },
    {
        ...
        "class_descriptions": (
            "activate card", "cancel transfer", "supported countries", "forgotten passcode"
        ),
    },
    {
        ...
        "class_descriptions": ("entity", "description", "human", "numeric answer"),
    },
)
...
def choose_verbalizers(tokenizer, spec: dict) -> tuple[tuple[str, ...], tuple[int, ...]]:
    ...
    for word in CHOICE_WORDS:
        ...
        if len(encoded) != 1:
            raise RuntimeError(f"choice {word} is not one tokenizer token")
        token_ids.append(int(encoded[0]))
    if len(set(token_ids)) != 4:
        raise RuntimeError("multiple-choice token ids are not unique")
    return CHOICE_WORDS, tuple(token_ids)
...
def encode_rows(tokenizer, spec: dict, task_index: int, rows,
    ...
    examples = []
    options = ", ".join(
        f"{words[index]}={description}"
        for index, description in enumerate(spec["class_descriptions"])
    )
    for _, text, target in rows:
        prefix = (
            f'Classify {spec["name"]}. Options: [{options}]. '
            f'Reply with one letter only. Text: '
        )
        suffix = "\nAnswer:"
...
def build_tasks(tokenizer, keys_per_task: int):
        ...
        declarations.append(
            {
                ...
                "class_descriptions": spec["class_descriptions"],
    ...
    declaration = {
        "protocol": "hf_public_choice_v1",
        "train_per_class": TRAIN_PER_CLASS,
        "eval_per_class": EVAL_PER_CLASS,
        "tasks": declarations,
    }
    metadata = {
        "suite": "hf_public_choice_v1", "num_labels": 4,
        "label_ids": [0, 1, 2, 3],
...
def main() -> None:
    ...
    parser.add_argument("--updates-per-task", type=int, default=1024)
