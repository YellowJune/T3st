...
def pack_residual(m_sem, v_sem, residual):
    ...
    v_phys = f32(jnp.bitwise_or(vb, jnp.bitwise_and(jnp.right_shift(rb, jnp.uint32(16)), LOW)))
...
def gradient_for(n, step):
    ...
    return jnp.sin(idx * jnp.float32(0.0017) + jnp.float32(step * 0.13)) * jnp.float32(0.01)


def semantic_adam(param, m_phys, v_phys, grad, step, lr=3e-4):
    b1 = jnp.float32(0.9); b2 = jnp.float32(0.999)
    m = decode_semantic(m_phys); v = decode_semantic(v_phys)
    ...
    m_use = decode_semantic(m_can); v_use = decode_semantic(v_can)
    bc1 = jnp.float32(1.0 - 0.9**step)
    bc2 = jnp.float32(1.0 - 0.999**step)
...
def exactness(n: int, steps: int, stride: int):
    ...
    roundtrip = bool(np.array_equal(np.asarray(jax.device_get(bits(probe))),
                                    np.asarray(jax.device_get(bits(recovered)))))
    semantic_clean = bool(np.array_equal(np.asarray(jax.device_get(bits(decode_semantic(mp)))),
                                         np.asarray(jax.device_get(bits(z)))))
    ...
    for step in range(steps):
        ...
        # Synchronize both paths before equality audit.
        ext[0].block_until_ready(); dfc[0].block_until_ready()
        pe, me, ve, re = ext
        pd, md, vd = dfc
        checks = [
            np.array_equal(np.asarray(jax.device_get(bits(pe))), np.asarray(jax.device_get(bits(pd)))),
            np.array_equal(np.asarray(jax.device_get(bits(decode_semantic(me)))), np.asarray(jax.device_get(bits(decode_semantic(md)))),
            np.array_equal(np.asarray(jax.device_get(bits(decode_semantic(ve)))), np.asarray(jax.device_get(bits(decode_semantic(vd)))),
            np.array_equal(np.asarray(jax.device_get(bits(re))), np.asarray(jax.device_get(bits(decode_residual(md,vd)))),
        ]
        if not all(checks):
            raise AssertionError(f"external/DFC trajectory mismatch at step {step}")
    return {
        'coordinates': n, 'steps': steps, 'stride': stride,
        'roundtrip_bitwise': roundtrip, 'semantic_invariance_bitwise': semantic_clean,
...
def timing(n: int, steps: int, stride: int):
    p0 = jnp.zeros(n, dtype=jnp.float32); z = jnp.zeros_like(p0)
    ext = (p0,z,z,z); dfc=(p0,z,z)
    ext_step = jax.jit(external_step, static_argnums=(2,)); d_step = jax.jit(dfc_step, static_argnums=(2,))
    # Compile/warm independently, excluded from timings.
    ext = ext_step(ext, 0, stride); ext[0].block_until_ready()
    dfc = d_step(dfc, 0, stride); dfc[0].block_until_ready()
    t0=time.perf_counter()
    for i in range(1,steps+1): ext=ext_step(ext,i,stride)
    ext[0].block_until_ready(); te=time.perf_counter()-t0
    t0=time.perf_counter()
    for i in range(1,steps+1): dfc=d_step(dfc,i,stride)
    dfc[0].block_until_ready(); td=time.perf_counter()-t0
    return {
        'coordinates': n, 'steps': steps,
        'external_seconds': te, 'dfc_seconds': td,
...
def multi_device_probe(local_n: int, stride: int):
    ...
    # collective communication after identical sparsification. DFC payload is
    # local state and never enters the communicated tensor.
    per = jnp.zeros((len(devices), local_n), dtype=jnp.float32)
    residual = jnp.zeros_like(per)
    m = jnp.zeros_like(per); v=jnp.zeros_like(per)
    ...
    return {
        'devices':len(devices),'local_coordinates':local_n,'global_coordinates':local_n*len(devices),
        'collective':'lax.psum','stride':stride,'reduced_sha256':hash_array(reduced),
        'residual_sha256':hash_array(rnew),'pass':True,
    }
...
def main():
    ...
    a=ap.parse_args()
    result={
        'schema_version':1,'protocol':'dfc-ef-jax-xla-cross-substrate-v1',
    ...
    result['timing']=[timing(int(n),a.timing_steps,a.stride) for n in a.timing_sizes.split(',') if n.strip()]
